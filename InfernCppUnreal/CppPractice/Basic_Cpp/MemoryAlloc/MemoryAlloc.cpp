...
public:
	...
	// 최상위 소멸자에 virtual을 붙이면 하위는 전부 소멸자가 가상함수로 변환됨
	// 가상함수 테이블이 만들어지면서 item 소멸자를 호출하면 원본 타입의 소멸자도 호출
	// virtual을 안 붙이면 일부 소멸자만 호출되는 케이스가 생김
	//부모 - 자식 관계에서 부모 클래스 소멸자에서는 까먹지말고 virtual을 붙이자!
	virtual ~Item()
	{
		cout << "~Item()" << endl;
	}
